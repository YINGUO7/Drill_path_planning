...
class MillingEnvNurbs(gym.Env):
    ...
    def __init__(self, wp_size_m=210e-3, pocket_side_m=190e-3, corner_radius_m=10e-3,
        ...
        from speedplanner import SpeedPlanner
        from renderer import MillingEnvRenderer
        self.speed_planner = SpeedPlanner(self)
        self.renderer = MillingEnvRenderer(self)
    ...
    def estimate_machining_time(self, path, mode="local", return_profile=False, local_window_length=None,
                                planner=None):
        """调用独立速度规划模块计算加工时间。"""
        return self.speed_planner.estimate_machining_time(
            path, mode=mode, return_profile=return_profile,
            local_window_length=local_window_length, planner=planner
        )

    def _full_time_reward(self, full_time):
        """调用独立速度规划模块计算完整路径时间奖励。"""
        return self.speed_planner._full_time_reward(full_time)

    def _estimate_machining_time_fast(self, path, mode="local", return_profile=False, local_window_length=None):
        """调用独立速度规划模块中的fast局部时间估计。"""
        return self.speed_planner._estimate_machining_time_fast(
    ...
    def _full_path_speed_diagnostics(self):
        """调用独立速度规划模块生成完整路径速度诊断。"""
        return self.speed_planner._full_path_speed_diagnostics()

    def _estimate_local_window_time(self, current_path):
        """调用独立速度规划模块估计局部窗口时间。"""
        return self.speed_planner._estimate_local_window_time(current_path)
    ...
    def _record_render_history(self, info, reward):
        """记录render诊断历史；实际实现位于Renderer.py。"""
        return self.renderer._record_render_history(info, reward)
    ...
    def render(self, force_speed_diagnostics=False):
        """刷新render窗口；实际实现位于Renderer.py。"""
        return self.renderer.render(force_speed_diagnostics=force_speed_diagnostics)

    def close(self):
        """关闭render窗口；实际实现位于Renderer.py。"""
